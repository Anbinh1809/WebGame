...
describe('instanced Poly Haven tree selection', () => {
  ...
  it('keeps real-tree instance counts bounded separately from texture-pack resolution', () => {
    expect(treeModelInstanceLimit('low', 512)).toBe(32)
    expect(treeModelInstanceLimit('medium', 512)).toBe(64)
    expect(treeModelInstanceLimit('high', 512)).toBe(128)
    expect(treeModelInstanceLimit('ultra', 512)).toBe(256)
    expect(treeModelInstanceLimit('high', 7)).toBe(7)
    expect(treeModelInstanceLimit('low', 4)).toBe(4)
    expect(treeModelInstanceLimit('medium', 4)).toBe(4)
    expect(treeModelInstanceLimit('high', 4)).toBe(4)
    expect(treeModelInstanceLimit('low', 8)).toBe(8)
    expect(treeModelInstanceLimit('medium', 8)).toBe(8)
  ...
  it('selects a bounded, locally packaged boulder without treating it as foliage', () => {
    ...
    expect(rock?.runtimeBudget.maxInstances).toBe(64)
