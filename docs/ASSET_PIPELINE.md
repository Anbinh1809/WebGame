...
| `wooden_lantern_01` | CC0-1.0 | active village lantern replacing primitive glow spheres, 3,430 triangles/model | 3,768,780 | 3,539,480 | `web-1k`, `desktop-2k`, `desktop-4k`, `cinema-8k` |
| `wooden_barrels_01` | CC0-1.0 | active workshop stockpile, 23,946 triangles/model | 7,490,847 | 6,508,060 | `web-1k`, `desktop-2k`, `desktop-4k`, `cinema-8k` |
...
Total initial Web 1K preload: **12,044,474 bytes** (11.49 MiB), within the 25 MiB budget. After `/play`, the active detailed cloud sky (**1,744,648 bytes**), environment GLBs (**14,979,340 bytes** for the 1K tree, fern, shoreline-rock, and boulder set), and settlement GLBs (**10,047,540 bytes** for lanterns and stockpiles) load on demand. Aetheria retains deterministic instancing and procedural fallback meshes for a missing/corrupt model rather than cloning hero geometry per tile. Water is a dynamic physical surface that reflects the local Poly Haven HDRI; Poly Haven's texture catalog currently has no tileable liquid PBR material to package in its place.
...
For the two village models, run `npm.cmd run assets:settlement:web-1k` for Web 1K and `npm.cmd run assets:settlement:<pack>` for each desktop pack after the material/environment command in the table. Verify them with `npm.cmd run assets:verify:settlement -- --pack <pack>`.

The current verified material/HDRI pack sizes are 18,886,320 bytes (`web-1k`, including deferred maps and Cloud Layers), 74,411,653 bytes (`desktop-2k`), 267,443,784 bytes (`desktop-4k`), and 906,447,263 bytes (`cinema-8k`). The four active environment GLBs add 14,979,340 bytes (1K), 47,398,340 bytes (2K), 158,250,436 bytes (4K), and 535,883,856 bytes (8K); the two active settlement GLBs add 10,047,540 bytes (1K), 36,598,444 bytes (2K), 138,176,776 bytes (4K), and 522,568,868 bytes (8K). Only one selected pack is loaded at a time.
